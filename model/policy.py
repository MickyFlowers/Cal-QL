...
class ResNetPolicy(nn.Module):
    ...
    def setup(self):
        ...
        self.spatial_pooler = nn.Conv(
            2048, kernel_size=(7, 7), strides=(1, 1), padding='VALID'
        )
        if self.orthogonal_init:
            self.bottleneck = nn.Dense(self.hidden_dim, kernel_init=jax.nn.initializers.orthogonal(jnp.sqrt(2.0)), bias_init=jax.nn.initializers.zeros)
        else:
    ...
    def __call__(self, observations, images, deterministic=False, repeat=None):
        ...
        image_features = self.resnet(images).last_hidden_state # bchw
